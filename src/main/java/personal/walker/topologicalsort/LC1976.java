...
public class LC1976 implements Solution {
    ...
    class Distance {
        ...
        public Distance(int _target, long _cost) {
            this.target = _target;
            this.cost = _cost;
        }
    }
    ...
    // dp[0][n] = dp[1][n-1] =
    public int countPaths(int n, int[][] srcRoads) {
        List<List<Distance>> graph = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            graph.add(new ArrayList<>());
        }
        for (int[] road : srcRoads) {
            graph.get(road[0]).add(new Distance(road[1], road[2]));
            graph.get(road[1]).add(new Distance(road[0], road[2]));
        }
        long[] resultDis = new long[n];

        Arrays.fill(resultDis, Long.MAX_VALUE);
        resultDis[0] = 0;
        long[] result = new long[n];
        boolean[] visited = new boolean[n];
        PriorityQueue<Distance> pq = new PriorityQueue<>((o1, o2) -> (int) (o1.cost - o2.cost));
        pq.add(new Distance(0, 0));
        result[0] = 1;
        while (!pq.isEmpty()) {
            Distance distance = pq.poll();
          //  visited[distance.target] = true;
            /**
             * 还是不太理解这一行代码：
             */
            if (resultDis[distance.target] < distance.cost){
                continue;
            }
            for (Distance neighbor : graph.get(distance.target)) {
//                if (visited[neighbor.target]) {
//                    continue;
//                }

                if (resultDis[neighbor.target] > neighbor.cost + resultDis[distance.target]) {
                    resultDis[neighbor.target] = neighbor.cost + resultDis[distance.target];
                    pq.offer(new Distance(neighbor.target, resultDis[neighbor.target]));
                    // tricky at this
                    result[neighbor.target] = result[distance.target];
                }else if (resultDis[neighbor.target] == neighbor.cost + resultDis[distance.target]){
                    // and tricky at this line too...
                    result[neighbor.target] = (result[neighbor.target] + result[distance.target]) % 1000000007;
                }
            }
            if (distance.target == n - 1) {
                break;
            }
        }
        System.out.println(Arrays.toString(result));
        System.out.println(Arrays.toString(resultDis));

        return (int)result[n-1] % 1000000007;
    }
}
